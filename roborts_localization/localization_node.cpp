...
bool LocalizationNode::Init() {
  ...
  localization_config.GetParam(&nh_);//ros param实现h文件读取参数服务器

  odom_frame_   = std::move(localization_config.odom_frame_id);//move转移提高性能
  ...
  // Use message filter for time synchronizer (laser scan topic and tf between odom and base frame)将消息缓存直到可以转换到对应坐标系
  ...
  initial_pose_sub_ = nh_.subscribe("initialpose", 2, &LocalizationNode::InitialPoseCallback, this);//订阅预测到的位子信息并处理
...
bool LocalizationNode::GetLaserPose() {
  ...
  Vec3d laser_pose;
  laser_pose.setZero();
  GetPoseFromTf(base_frame_, laser_scan_msg->header.frame_id, ros::Time(), laser_pose);//得到雷达的信息
...
void LocalizationNode::LaserScanCallback(const sensor_msgs::LaserScan::ConstPtr &laser_scan_msg_ptr){
  ...
  Vec3d pose_in_odom;
  if(!GetPoseFromTf(odom_frame_, base_frame_, last_laser_msg_timestamp_, pose_in_odom))//得到里程计中的位子
  {
  ...
  TransformLaserscanToBaseFrame(angle_min, angle_increment, laser_scan_msg);//得到开始扫描的角度和每一次的增量

  amcl_ptr_->Update(pose_in_odom,
                    laser_scan_msg,
                    angle_min,
                    angle_increment,
                    particlecloud_msg_,//一列pose
                    hyp_pose_); //预测pose 以及可能的噪声矩阵

  LOG_ERROR_IF(!PublishTf()) << "Publish Tf Error!";//发布 odom到map
...
void LocalizationNode::PublishVisualize(){
  ...
  if(pose_pub_.getNumSubscribers() > 0){//如果有连接到此publisher的订阅者 预测位子
    ...
    pose_msg_.pose.orientation = tf::createQuaternionMsgFromYaw(hyp_pose_.pose_mean[2]);//从yaw得到车头朝向
    pose_pub_.publish(pose_msg_);//具有时间戳和自带坐标的位姿信息
  }
  ...
  if(particlecloud_pub_.getNumSubscribers() > 0){
    ...
    particlecloud_pub_.publish(particlecloud_msg_);//如果有订阅者，也发布它对应的位姿信息 粒子云
  }

  if(!publish_first_distance_map_) {//
    distance_map_pub_.publish(amcl_ptr_->GetDistanceMapMsg());//distance map ??
...
bool LocalizationNode::PublishTf() {
  ...
  if (amcl_ptr_->CheckTfUpdate()) {
    ...
    try {
      tf::Transform tmp_tf(tf::createQuaternionFromYaw(hyp_pose_.pose_mean[2]),//机器人预测位子朝向
                           tf::Vector3(hyp_pose_.pose_mean[0],//位置
                                       hyp_pose_.pose_mean[1],
                                       0.0));//base link 和map的关系   amcl预测到
      tf::Stamped<tf::Pose> tmp_tf_stamped(tmp_tf.inverse(),//map 和baselink的关系
                                           last_laser_msg_timestamp_,
                                           base_frame_);
      this->tf_listener_ptr_->transformPose(odom_frame_,//监听到baselink和odom的关系 从而得到map和odom的关系
                                            tmp_tf_stamped,
                                            odom_to_map);
    } catch (tf::TransformException &e) {
    ...
    tf::StampedTransform tmp_tf_stamped(latest_tf_.inverse(),//map到odom
                                        transform_expiration,//时间
                                        global_frame_,//map
                                        odom_frame_);//odom
    this->tf_broadcaster_ptr_->sendTransform(tmp_tf_stamped);//发布
    sent_first_transform_ = true;
    return true;
  } else if (latest_tf_valid_) {//上一帧的tfOK
                   ...
                   Vec3d &pose)
{
  ...
  try {
    this->tf_listener_ptr_->transformPose(target_frame,//ident 在 target_frame表示为posestamp
                                          ident,
                                          pose_stamp);
  } catch (tf::TransformException &e) {
