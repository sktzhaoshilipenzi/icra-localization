...
template<typename T>
inline T RandomGaussianNum(T cov) {//高斯数 输入值方差
	T sigma = std::sqrt(std::abs(cov));//标准差
	T x1, x2, w, r;
	do {
		do {
			r = drand48();//从零到一的随机数
 ...
 */
template<typename T>
inline T RandomGaussianNumByStdDev(T sigma) {//输入值标准差
	T x1, x2, w, r;
	do {
		do {
			r = drand48();
...
T EuclideanDistance(T x1, T y1, T x2, T y2) {
	return std::sqrt(std::pow((x1 - x2), 2) + std::pow((y1 - y2), 2));//两点距离
}

template<typename T>
inline bool Near(T num1, T num2, T bound) {//判断两数相加是否等于第三数
	if (num1 > num2 + bound) {
		return false;
...
inline Mat3d MsgCovarianceToMat3d(const boost::array<double, 36> &msg_cov) {//提取矩阵的一部分
	Mat3d pose_cov;
	pose_cov.setZero();
